...
APPROVAL_TEMPLATE = '''
<div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; background: #030712; padding: 30px; border-radius: 12px; color: #f8fafc;">
    ...
    <div>
        ...
        <p style="color: #cbd5e1; line-height: 1.6;">A sua conta na <strong>BF Digital Exchange</strong> foi validada e <strong style="color: #22c55e;">aprovada</strong> com sucesso.</p>
    </div>
</div>
'''
...
<div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; background: #030712; padding: 30px; border-radius: 12px; color: #f8fafc;">
    ...
    <div>
        <p style="font-size: 16px;"><strong>Olá, {{ nome }},</strong></p>
        <p style="color: #cbd5e1; line-height: 1.6;">O seu pedido de registo na <strong>BF Digital Exchange</strong> foi <strong style="color: #ef4444;">negado</strong> neste momento.</p>
    </div>
</div>
'''
...
def register():
    ...
    try:
        ...
        cursor.execute("INSERT INTO users (nome, email, telefone, data_nascimento, password, status) VALUES (?, ?, ?, ?, ?, 'Pendente')",
                       (nome, email, telefone, data_nascimento_str, hashed_password))
        conn.commit()
        conn.close()
        
        try:
            msg_admin = Message(f"[NOVO REGISTO] Cliente: {nome}", recipients=['bfdigital53@gmail.com'])
            msg_admin.body = f"Novo registo submetido:\n\nNome: {nome}\nE-mail: {email}\nTelefone: {telefone}"
            mail.send(msg_admin)
        except Exception:
            pass
...
def login():
    ...
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM users WHERE email = ? AND telefone = ?", (email, telefone))
    user = cursor.fetchone()
    conn.close()
    
    if user and check_password_hash(user[7], password_plain):
...
def create_admin():
    ...
    try:
        ...
        conn.commit()
        conn.close()
        flash("Novo gestor criado com sucesso!", "success")
    except Exception:
...
def update_rates():
    ...
    conn.commit()
    conn.close()
    flash(f"Cotações para {par} atualizadas com sucesso!", "success")
    return redirect(url_for('admin_panel'))
...
def edit_user(user_id):
    ...
    if password_plain and not password_plain.startswith('pbkdf2:'):
        hashed_password = generate_password_hash(password_plain)
        cursor.execute("UPDATE users SET nome = ?, email = ?, telefone = ?, status = ?, password = ? WHERE id = ?", (nome, email, telefone, status, hashed_password, user_id))
    else:
        cursor.execute("UPDATE users SET nome = ?, email = ?, telefone = ?, status = ? WHERE id = ?", (nome, email, telefone, status, user_id))
...
def delete_user(user_id):
    ...
    conn.commit()
    conn.close()
    flash("Registo de cliente eliminado com sucesso.", "success")
    return redirect(url_for('admin_panel'))
...
def recover():
    if request.method == 'POST':
        email = request.form['email']
        telefone = request.form['telefone']
        nova_password_plain = request.form['nova_password']
        hashed_password = generate_password_hash(nova_password_plain)
        
        conn = sqlite3.connect('database.db')
        cursor = conn.cursor()
        cursor.execute("UPDATE users SET password = ? WHERE email = ? AND telefone = ?", (hashed_password, email, telefone))
        conn.commit()
